...
Сервер был оставлен запущенным в фоне (task id **`bj63h12j5`**, порт **5099**) чтобы пользователь мог потрогать. Возможно ещё работает — проверь `lsof -i:5099` или `TaskStop`.
...
- Ollama `format` с JSON Schema — модели могут «застрять» (по слухам) на сложных схемах. Простую (3 элемента, два поля) mistral держит уверенно. Если в будущем появятся вложенные схемы — может понадобиться ретрай или фолбэк на `format: "json"`.
